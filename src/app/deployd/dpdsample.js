...
function(a, b) {
    ...
    c.decodePacket = function(a) {
        ...
        if (a.charAt(0) == "ï¿½") {
            var b = [];
            for (var d = 1, e = ""; d < a.length; d++) a.charAt(d) == "ï¿½" ? (b.push(c.decodePacket(a.substr(d + 1).substr(0, e))), d += Number(e) + 1, e = "") : e += a.charAt(d);
            return b
        }
...
function(a, b, c) {
    ...
    a.Socket = d, b.util.mixin(d, b.EventEmitter), d.prototype.of = function(a) {
        ...
        for (var e = 0, f; f = c[e]; e++)
            if (b.Transport[f] && b.Transport[f].check(this) && (!this.isXDomain() || b.Transport[f].xdomainCheck())) return new b.Transport[f](this, this.sessionid);
        return null
...
dpd.actions = dpd("/dpd/actions");
dpd.actions.on = function(ev, fn) {
    return dpd.on("dpd/actions" + ":" + ev, fn);
}
dpd.actions.once = function(ev, fn) {
    return dpd.once("dpd/actions" + ":" + ev, fn);
}
dpd.actions.off = function(ev, fn) {
    return dpd.off("dpd/actions" + ":" + ev, fn);
}
dpd.actions.rename = function(path, body, fn) {
    dpd.actions.exec("rename", path, body, fn);
}

dpd.data = dpd("/dpd/data");
dpd.data.on = function(ev, fn) {
    return dpd.on("dpd/data" + ":" + ev, fn);
}
dpd.data.once = function(ev, fn) {
    return dpd.once("dpd/data" + ":" + ev, fn);
}
dpd.data.off = function(ev, fn) {
    return dpd.off("dpd/data" + ":" + ev, fn);
}
dpd.data.rename = function(path, body, fn) {
    dpd.data.exec("rename", path, body, fn);
}

dpd.languages = dpd("/dpd/languages");
dpd.languages.on = function(ev, fn) {
    return dpd.on("dpd/languages" + ":" + ev, fn);
}
dpd.languages.once = function(ev, fn) {
    return dpd.once("dpd/languages" + ":" + ev, fn);
}
dpd.languages.off = function(ev, fn) {
    return dpd.off("dpd/languages" + ":" + ev, fn);
}
dpd.languages.rename = function(path, body, fn) {
    dpd.languages.exec("rename", path, body, fn);
}

dpd.layouts = dpd("/dpd/layouts");
dpd.layouts.on = function(ev, fn) {
    return dpd.on("dpd/layouts" + ":" + ev, fn);
}
dpd.layouts.once = function(ev, fn) {
    return dpd.once("dpd/layouts" + ":" + ev, fn);
}
dpd.layouts.off = function(ev, fn) {
    return dpd.off("dpd/layouts" + ":" + ev, fn);
}
dpd.layouts.rename = function(path, body, fn) {
    dpd.layouts.exec("rename", path, body, fn);
}

dpd.migrations = dpd("/dpd/migrations");
dpd.migrations.on = function(ev, fn) {
    return dpd.on("dpd/migrations" + ":" + ev, fn);
}
dpd.migrations.once = function(ev, fn) {
    return dpd.once("dpd/migrations" + ":" + ev, fn);
}
dpd.migrations.off = function(ev, fn) {
    return dpd.off("dpd/migrations" + ":" + ev, fn);
}
dpd.migrations.rename = function(path, body, fn) {
    dpd.migrations.exec("rename", path, body, fn);
}

dpd.nodes = dpd("/dpd/nodes");
dpd.nodes.on = function(ev, fn) {
    return dpd.on("dpd/nodes" + ":" + ev, fn);
}
dpd.nodes.once = function(ev, fn) {
    return dpd.once("dpd/nodes" + ":" + ev, fn);
}
dpd.nodes.off = function(ev, fn) {
    return dpd.off("dpd/nodes" + ":" + ev, fn);
}
dpd.nodes.rename = function(path, body, fn) {
    dpd.nodes.exec("rename", path, body, fn);
}

dpd.settings = dpd("/dpd/settings");
dpd.settings.on = function(ev, fn) {
    return dpd.on("dpd/settings" + ":" + ev, fn);
}
dpd.settings.once = function(ev, fn) {
    return dpd.once("dpd/settings" + ":" + ev, fn);
}
dpd.settings.off = function(ev, fn) {
    return dpd.off("dpd/settings" + ":" + ev, fn);
}
dpd.settings.rename = function(path, body, fn) {
    dpd.settings.exec("rename", path, body, fn);
}

dpd.texts = dpd("/dpd/texts");
dpd.texts.on = function(ev, fn) {
    return dpd.on("dpd/texts" + ":" + ev, fn);
}
dpd.texts.once = function(ev, fn) {
    return dpd.once("dpd/texts" + ":" + ev, fn);
}
dpd.texts.off = function(ev, fn) {
    return dpd.off("dpd/texts" + ":" + ev, fn);
}
dpd.texts.rename = function(path, body, fn) {
    dpd.texts.exec("rename", path, body, fn);
}

dpd.users = dpd("/dpd/users");
...
dpd.users.on = function(ev, fn) {
    return dpd.on("dpd/users" + ":" + ev, fn);
}
dpd.users.once = function(ev, fn) {
    return dpd.once("dpd/users" + ":" + ev, fn);
}
dpd.users.off = function(ev, fn) {
    return dpd.off("dpd/users" + ":" + ev, fn);
}